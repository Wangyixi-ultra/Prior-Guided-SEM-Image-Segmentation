...
# -*- coding: utf-8 -*-
"""
极简 nnUNet 预测流水线（yolo）
在文件内部即可改路径，无需命令行参数
"""

import os
os.environ['NUMEXPR_MAX_THREADS'] = '32'  # 避免 numexpr 线程限制警告

import cv2, subprocess, shutil, json
...
# ========== 1. 用户只改这里 ==========
INPUT_DIR   = Path('/home/chen/seg6/predict_no_label/bandgap')  # 原始图像
OUTPUT_DIR  = Path('/home/chen/seg6/predict_no_label/bandgap/out123')      # nnUNet 预测结果
BORDER_DIR  = Path('/home/chen/seg6/predict_no_label/bandgap/border123')   # 轮廓叠加图
JSON_DIR    = Path('/home/chen/seg6/predict_no_label/bandgap/json123')                 # JSON标注文件输出目录
GRAB_IMG_DIR = Path('/home/chen/seg6/predict_no_label/bandgap/image_for_grab')         # 供 grab.py 匹配用的重命名灰度图
# nnUNet 参数
DATASET_ID  = 123
CONFIG      = '2d'
FOLD        = 0 
TRAINER     = 'nnUNetTrainerUMambaBotYoloSpatialCrossFusion'# (Code modified to iterate all trainers)
CHECKPOINT  = 'checkpoint_best.pth'
# ========== 2. 以下内容勿动 ========== 

CLASS_COLOR = {1: (75, 120, 235), 2: (230, 170, 90)}
...
def export_grayscale_for_grab(in_dir, grab_dir):
    """将输入图像按 case 序号重命名到新目录，供 grab.py 通过 case 名匹配灰度图。"""
    grab_dir.mkdir(exist_ok=True, parents=True)

    images = find_imgs(in_dir)
    if not images:
        print(f"未找到可导出的原始图像: {in_dir}")
        return
    ...
    for idx, f in enumerate(images):
        ...
        if img_original is None:
            print(f"[Warn] 无法读取文件，跳过导出: {f}")
            continue

        case_name = f'case{idx:03d}'
        gray = to1ch_uint8(img_original)
        cv2.imwrite(str(grab_dir / f'{case_name}.png'), gray)

    print(f"已导出重命名灰度图目录: {grab_dir}")
...
def prep_input(in_dir, tmp_dir):
    ...
    yolo_to_mask_map = {0: 2, 1: 1} # 0->ABO3->2, 1->PbI2->1
    ...
    for idx, f in enumerate(find_imgs(in_dir)):
        ...
        # Only needed if dataset expects it, but we generate it just in case because Dataset 122 needs it.
        # We always generate it to match the training data requirements we observed (Dataset 122).
        h, w = img_0000.shape
        if has_yolo:
            try:
...
def draw_contour(tmp_dir, out_dir, border_dir):
    ...
    for m in sorted(out_dir.glob('*.png')):
        ...
        for cls in np.unique(mask):
            ...
            cv2.drawContours(base,cnt,-1,CLASS_COLOR.get(cls,(255,255,255)),3)
...
def main():
    ...
    try:
        # 在预测前先导出 grab.py 可识别的命名格式
        export_grayscale_for_grab(INPUT_DIR, GRAB_IMG_DIR)
        ...
        for trainer_dir in sorted(dataset_dir.iterdir()):
            ...
            # 只跑指定的 Trainer（与上方 TRAINER 变量一致），三行注释掉即可遍历
            if trainer_name != TRAINER:
                print(f"跳过 {trainer_name} (非目标 Trainer)")
                continue
