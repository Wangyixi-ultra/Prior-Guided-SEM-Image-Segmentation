...
# ========== 用户配置 ==========
raw_img    = '/home/chen/seg6/raw/image_addzhouyuanyuan'          # 原始灰度图
raw_mask   = '/home/chen/seg6/raw/mask_only_gray_dir_nodefect_1class'  # 0/255/180/255 灰度 mask
...
test_names = {'02','06','10','14','22','24','33','37','38','41'}
...
def process_split(split, name_list):
    for name in tqdm(name_list, desc=f'Processing {split}'):
        ...
        # 如果mask尺寸不一致，强制缩放到原图大小(最近邻插值)
        if msk.shape != gray.shape:
            print(f"警告: Mask尺寸 {msk.shape} 与原图 {gray.shape} 不一致，正在调整 {name}...")
            msk = cv2.resize(msk, (gray.shape[1], gray.shape[0]), interpolation=cv2.INTER_NEAREST)
