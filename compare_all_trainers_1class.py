#!/usr/bin/env python3
"""
对Dataset109_Perovskite数据集下的所有trainer进行测试集对比评估
...
# ========== 配置 ==========
DATASET_NAME = "Dataset109_Perovskite"
...
def get_all_trainers():
    """获取Dataset109_Perovskite下所有的trainer模型"""
...
def visualize_errors_for_trainer(trainer_name, pred_folder):
    ...
    for filename in os.listdir(TEST_LABELS):
        if filename.endswith('.png'):
            ...
            # 为真实标签和预测结果绘制轮廓
            # 为每个类别绘制不同的轮廓
            for class_id in [1]:  # 一个类别: foreground
                # 创建掩码用于寻找轮廓
...
def main():
    print(f"{'='*80}")
    print("Dataset109_Perovskite 测试集对比评估 (优化版)")
    print("可视化: 绿色=foreground")
    print(f"{'='*80}")
